...
def sample_diffusion_chunks(dit: MotionDiT, cfg: dict, n: int,
    ...
    lat_std  = torch.tensor(np.maximum(np.array(cfg["latent_std"], dtype=np.float32), 1e-4),
                            device=device)
    z_unnorm = z_norm * lat_std + lat_mean   # (n, n_cond+H, D)
...
def main() -> None:
    ...
    # Latent stats for Gaussian baseline (always from the VAE dataset)
    ds_data  = np.load(LATENTS_ROOT / vae_run / "chunk_diffusion_dataset.npz")
    ...
    if args.mode in ("gaussian", "compare"):
        ...
        with torch.no_grad():
            zg = vae.sample(n, device) * lat_std_t + lat_mean_t
