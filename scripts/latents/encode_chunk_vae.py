...
Writes:
  storage/runs/<run>/encoded/<clip_stem>.npz
      z     : (T', latent_len, latent_dim)   per-window latents
      chunk : (T', H, D)                     normalised source chunks

  storage/runs/<run>/chunk_diffusion_dataset.npz
      latents    : (N, latent_len, latent_dim)
      states     : (N, H, D)
      clip_id    : (N,) int32
      clip_names : (n_clips,) str

  storage/runs/<run>/norm_stats.npz
      mean : (latent_dim,)   channel-wise mean of posterior means
      std  : (latent_dim,)   channel-wise std  of posterior means
...
def main() -> None:
    ...
    # Channel-wise stats across all samples and all temporal positions.
    STD_FLOOR   = 1e-4
    latent_mean = latents.reshape(-1, latents.shape[-1]).mean(axis=0)   # (latent_dim,)
    latent_std  = latents.reshape(-1, latents.shape[-1]).std(axis=0)    # (latent_dim,)
    n_floored   = int((latent_std < STD_FLOOR).sum())
    if n_floored:
        print(f"  [warn] {n_floored} latent channel(s) below std floor {STD_FLOOR}; clamped")
    ...
    print(f"  shape: {latent_mean.shape}  → {run_dir / 'norm_stats.npz'}")
